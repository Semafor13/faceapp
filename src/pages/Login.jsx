  ...
  if (imageError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-[24px] max-w-[840px] mx-auto">
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-blue-700 sm:text-4xl">
          <span className="block">
            Похоже для вашего профиля не найдено изображение.
          </span>
        </h2>
        <span className="block mt-4">
          Пожалуйста, свяжитесь с администрацией для регистрации или повторите попытку позже.
        </span>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col items-center justify-center gap-[24px] max-w-[720px] mx-auto">
      {!localUserStream && !modelsLoaded && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          <span className="block">
            Вы пытаетесь войти в систему под своим лицом.
          </span>
          <span className="block text-blue-600 mt-2">Загрузка моделей...</span>
        </h2>
      )}
      {!localUserStream && modelsLoaded && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          <span className="block text-blue-600 mt-2">
            Пожалуйста распознайте ваше лицо для дальнейшего входа
          </span>
        </h2>
      )}
      {localUserStream && loginResult === "SUCCESS" && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          <span className="block text-blue-600 mt-2">
            Ваше лицо успешно распознано!
          </span>
          <span className="block text-blue-600 mt-2">
            Пожалуйста подождите ещё {counter} секунд...
          </span>
        </h2>
      )}
      {localUserStream && loginResult === "FAILED" && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-blue-700 sm:text-4xl">
          <span className="block mt-[56px]">
            К сожалению распознать ваше лицо не вышло или вы вошли не под своим профилем.
          </span>
        </h2>
      )}
      {localUserStream && !faceApiLoaded && loginResult === "PENDING" && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          <span className="block mt-[56px]">Сканируем ваше лицо...</span>
        </h2>
      )}
            ...
            {modelsLoaded ? (
              <>
                <button
                  ...
                  type="button"
                  className="flex justify-center items-center w-full py-2.5 px-5 mr-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg border border-gray-200 inline-flex items-center"
                >
                  Просканировать моё лицо
                </button>
              </>
            ) : (
              <>
                ...
                >
                  ...
                  Пожалуйста дождитесь загрузки моделей...
                </button>
              </>
            )}
          </>
